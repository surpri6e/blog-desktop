...
const SettingsPage = (): JSX.Element => {
  ...
  const [value, loading, error] = useDocumentData<IFirebase>(
    doc(
      database,
      'users',
      user?.displayName ? user.displayName : user?.uid ? user.uid : ' '
    ) as DocumentReference<IFirebase>
  )
            ...
            {loadingUser || loading ? (
              ...
              <>
                ...
                <div className="settings_buttons">
                  <Link
                    to={`/a/${user?.displayName ? user.displayName : user.uid}`}
                    className="buttons buttons--red"
                  >
                    Отклонить
                  </Link>
